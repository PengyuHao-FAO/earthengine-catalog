...
class FilePathTest(unittest.TestCase):
  ...
  def test_bad_catalog_invalid_2_level(self):
    ...
    issues = list(Check.run(node))
    expect = [Check.new_issue(node, 'expected length 1 path: [\'A\', \'B\']')]
    self.assertEqual(expect, issues)
  ...
  def test_bad_2_level_collection(self):
    ...
    issues = list(Check.run(node))
    message = 'Col: expected one level path: A/A_B_C.json found: A/B/A_B_C.json'
    expect = [Check.new_issue(node, message)]
    self.assertEqual(expect, issues)
  ...
  def test_bad_collection(self):
    ...
    issues = list(Check.run(node))
    message = 'Col: expected one level path: A/A_B.json found: A/B.json'
    expect = [Check.new_issue(node, message)]
    self.assertEqual(expect, issues)
